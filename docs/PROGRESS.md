...
- **Gauntlets**: `pnpm v2:check` runs functional, web-preview, and terminal harnesses (or SKIP when a script is absent on older branches); individual commands remain `pnpm v2:gauntlet`, `pnpm v2:web-preview-gauntlet`, `pnpm v2:terminal-gauntlet`.
- **Default runtime**: `runAgentLoop` (unchanged).

## 2026-05-28 — v2 terminal tool gauntlet

- Added `fixtures/terminal-gauntlet` with offline `npm test` (no network/install).
- Added `scripts/v2-terminal-gauntlet.mjs` and root command `pnpm v2:terminal-gauntlet`.
- Deterministic coverage exercises `executeTool({ tool: "run_terminal" })` through `runTerminalInWorkspace` (extension terminal adapter).
- Evidence:
  - allowed `npm test` spawns with `shell: false` and passes;
  - `pnpm install` / `npm create` / `npx` require confirmation without spawn;
  - traversal, `.git`, and destructive commands blocked before spawn;
  - child env excludes `DEEPSEEK_API_KEY`, `TOKEN`, `SECRET`, `PASSWORD`, and `*_KEY`;
